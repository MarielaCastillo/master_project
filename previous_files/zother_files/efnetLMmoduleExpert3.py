...
class MultiModalDataset(Dataset):
    ...
    def __getitem__(self, index): ##collate #should it be reading one at a time???
        ...
        sample = {'point_cloud': torch_tensor, 'label': y_label}

        return sample

        #return [torch_tensor, y_label]
...
def collate_multimodal(queries):
    ...
    returns = {key: default_collate([d[key] for d in queries]) for key in queries[0]
               if key != 'point_cloud' and key != 'label'}  
    for input in queries:
        pcs.append(input['point_cloud'])
...
class LitModelEfficientNet(pl.LightningModule):
    ...
    def __init__(self, batch_size, transform):
        ...
        self.cnnexpertlidar = CNNExpert1(3, 3) 

    def forward(self, x1): ### here1
        hidden_statelidar, outlidar = self.cnnexpertlidar(x1)
        return outlidar
    ...
    def training_step(self, train_batch, batch_idx):
        inputlidar, labels = train_batch
        #optimizer.zero_grad()

        outputs = self(inputlidar) #forward(x1, x2, x3, x4)
